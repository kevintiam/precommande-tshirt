...
import { lireCommande, STATUTS, MOYENS } from '@/libs/commandes';
import { formatPrice } from '@/libs/currency';
import { INTERAC_EMAIL } from '@/components/PaymentMarks';

export const dynamic = 'force-dynamic';

// `texte` reçoit le nom du moyen choisi : parler de virement Interac à
// quelqu'un qui a payé par PayPal l'enverrait chercher au mauvais
// endroit. Toutes les entrées sont des fonctions, même celles qui
// n'utilisent pas l'argument — un seul appel côté rendu.
const AFFICHAGE = {
  [STATUTS.PAYEE]: {
    Icone: CheckCircle2,
    ...
    titre: 'Paiement reçu',
    texte: () =>
      'Merci ! Ta commande est confirmée. Retrait sur place pendant le camp.',
  },
  [STATUTS.EN_ATTENTE]: {
    Icone: Clock,
    ...
    texte: (moyen) =>
      `Envoie ton paiement ${moyen} en indiquant la référence ci-dessous, puis joins ta capture depuis l’écran de commande.`,
  },
  [STATUTS.A_VERIFIER]: {
    Icone: Hourglass,
    ...
    texte: (moyen) =>
      `Nous avons bien reçu ta capture. Ta commande sera confirmée dès que le paiement ${moyen} aura été vérifié de notre côté.`,
  },
  [STATUTS.ECHOUEE]: {
    Icone: XCircle,
...
const NOM_MOYEN = {
  [MOYENS.INTERAC]: 'Interac',
  [MOYENS.PAYPAL]: 'PayPal',
};
...
export default async function PageCommande({ params }) {
  ...
  const nomMoyen = NOM_MOYEN[commande.moyen] ?? NOM_MOYEN[MOYENS.INTERAC];
  ...
  return (
    <main className="mx-auto w-full max-w-lg flex-1 px-5 py-16">
      ...
      <div className="mt-8 rounded-xl border border-stone-200 p-5 text-sm">
        ...
        <div className="mt-1.5 flex justify-between text-stone-500">
          <span>Paiement</span>
          <span className="text-stone-700">{nomMoyen}</span>
        </div>
